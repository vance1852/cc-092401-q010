...
- `service.py`：角色权限、批次状态机、幂等导入、排除复核、任务租约与心跳续租、死信处置、审批和报告；
...
系统已经实现以下主流程：协议发布后不可原地覆盖；批次按版本从草稿进入运行、封存、分析和决定状态；观测分片同时受请求幂等键和来源行唯一身份保护；排除请求必须由不同角色复核；分析任务使用 SQLite 租约避免重复执行，持有者凭单调递增的 fencing 凭证在租约到期前心跳续租（续约只延长不缩短），租约过期后由其他工作进程接管；每次失败（含租约过期）都会记录摘要，达到协议 `max_analysis_attempts` 或系统默认上限后任务进入死信，统计负责人可以查看失败摘要、带理由重新入队或永久取消，已接管、死信或取消的任务不会被旧凭证复活；同一输入快照使用固定算法版本和随机种子得到一致结果；分析者与审批人职责分离，报告保留输入摘要、统计规则、任务状态和批次审计链。所有调度判断都通过可注入时钟完成，测试无需 sleep。
